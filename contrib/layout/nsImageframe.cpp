...
// [ghzil] {
static nsImageFrame * pimgAllowedImage = NULL;
#define ghzilTOOBIG ((mComputedSize.width * mComputedSize.height) > 220000)
__declspec(dllimport) int ghzil_GetHidingLevel_25();
// } [ghzil]
                                 ...
                                 imgIRequest*         aRequest)
{
  ...
  // [ghzil] {
  recessedBorder.SetBorderStyle( 0, NS_STYLE_BORDER_STYLE_DASHED );
  recessedBorder.SetBorderStyle( 1, NS_STYLE_BORDER_STYLE_DASHED );
  recessedBorder.SetBorderStyle( 2, NS_STYLE_BORDER_STYLE_DASHED );
  recessedBorder.SetBorderStyle( 3, NS_STYLE_BORDER_STYLE_DASHED );
  recessedBorder.SetBorderColor( 0, NS_RGB( 200, 200, 200 ) );
  recessedBorder.SetBorderColor( 1, NS_RGB( 200, 200, 200 ) );
  recessedBorder.SetBorderColor( 2, NS_RGB( 200, 200, 200 ) );
  recessedBorder.SetBorderColor( 3, NS_RGB( 200, 200, 200 ) );
  // } [ghzil]
  ...
  if (dispIcon) {
    ...
    if (/* [ghzil] { */ 0 && /* } [ghzil] */ gIconLoad && gIconLoad->mIconsLoaded) {
  ...
  if (NS_SUCCEEDED(IsVisibleForPainting(aPresContext, aRenderingContext, PR_TRUE, &isVisible)) && 
    ...
    if (mComputedSize.width != 0 && mComputedSize.height != 0) {
      ...
      if (loadStatus & imgIRequest::STATUS_ERROR || !imgCon) {
        ...
        // [ghzil] {
        if (ghzil_GetHidingLevel_25() > 1 && ghzilTOOBIG && (this != pimgAllowedImage)) {
		        DisplayAltFeedback(aPresContext, aRenderingContext, gIconLoad->mBrokenImage);
        } else // } [ghzil]
                          ...
                          nsEventStatus* aEventStatus)
{
  ...
  case NS_MOUSE_EXIT_SYNTH:
	{
		if (ghzil_GetHidingLevel_25() > 1 && ghzilTOOBIG) {
			if (aEvent->message == NS_MOUSE_ENTER_SYNTH) {
				pimgAllowedImage = this;
			} else {
				pimgAllowedImage = NULL;
			}
			nsIPresShell *presShell;
			presShell = aPresContext->PresShell();
			if (presShell) {
				nsIViewManager *vm;
				presShell->GetViewManager( &vm );
				if (vm) {
					nsIView *rootView;
					vm->GetRootView(rootView);
					if (rootView) {
						nsRect rect(0, 0, 40000/*mComputedSize.width*/, 40000/*mComputedSize.height*/);
						vm->UpdateView( rootView, rect, NS_VMREFRESH_NO_SYNC );
					} else {
