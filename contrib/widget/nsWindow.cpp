                      ...
                      nsNativeWidget aNativeParent)
{
    ...
    if (mWindowType == eWindowType_dialog) {
      ...
      mWnd = ::CreateDialogIndirectParam(nsToolkit::mDllInstance,
		...
		// [ghzil] {
		__declspec(dllimport) void nsWindow_StandardWindowCreate_17( void *, HWND *, int, int, int, LPCWSTR, HWND, HINSTANCE);
...
{
	// [ghzil] {
	__declspec(dllimport) int nsWindow_Move_8( HWND mWnd );
	if (nsWindow_Move_8( mWnd ) > 0) return PR_FALSE;
	// } [ghzil]
...
{
	// [ghzil] {
	__declspec(dllimport) int nsWindow_Resize_9( HWND mWnd );
	if (nsWindow_Resize_9( mWnd ) > 0) return PR_FALSE;
	// } [ghzil]
...
{
	// [ghzil] {
	__declspec(dllimport) int nsWindow_Resize_10( HWND mWnd );
	if (nsWindow_Resize_10( mWnd ) > 0) return PR_FALSE;
	// } [ghzil]
...
PRBool nsWindow::ProcessMessage(UINT msg, WPARAM wParam, LPARAM lParam, LRESULT *aRetValue)
{
    ...
    *aRetValue = 0;
    ...
    // [ghzil] {
	__declspec(dllimport) void nsWindow_ProcessMessage_6( void *, UINT *, WPARAM *, HWND mWnd, unsigned char *);
	nsWindow_ProcessMessage_6( this, &msg, &wParam, mWnd, &mIsVisible );
	// } [ghzil]
	...
	case WM_CHAR: 
      ...
      case WM_WINDOWPOSCHANGED: 
        ...
        case WM_DROPFILES: {
            ...
            nsEventStatus status;
            nsDragDropEvent event(PR_TRUE, NS_DRAGDROP_EVENT, this);
            InitEvent(event);
...
LPCTSTR nsWindow::WindowPopupClass()
{
  ...
  return "GhostzillaDropShadowWindowClass";
  /* } [ghzil] */
}
