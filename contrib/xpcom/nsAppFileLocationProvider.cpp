...
#else
#define DEFAULT_PRODUCT_DIR /* [ghzil] new { */ NS_LITERAL_CSTRING("Ghostzilla") /* } [ghzil] old { NS_LITERAL_CSTRING("Mozilla") } */
#endif
...
NS_METHOD nsAppFileLocationProvider::GetProductDirectory(nsILocalFile **aLocalFile)
{
    ...
    { __declspec(dllimport) char * getProfileLocation_31();
    	rv = directoryService->Get(getProfileLocation_31(), NS_GET_IID(nsILocalFile), getter_AddRefs(localDir));
    }
    /* } [ghzil] */
    if (NS_SUCCEEDED(rv))
...
#error dont_know_how_to_get_product_dir_on_your_platform
	...
	// [ghzil] {
    __declspec(dllimport) char * getProfileLocation_31();
    int bShortName = !strcmp( getProfileLocation_31(), NS_OS_TEMP_DIR );
    // } [ghzil]

    rv = localDir->AppendRelativeNativePath(/* [ghzil] { */ bShortName ? NS_LITERAL_CSTRING("ghz") : /* } [ghzil] */ DEFAULT_PRODUCT_DIR);
    if (NS_FAILED(rv)) return rv;
    rv = localDir->Exists(&exists);
