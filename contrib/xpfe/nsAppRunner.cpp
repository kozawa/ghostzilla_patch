...
#if 1 // [ghzil]: old: defined(XP_UNIX) && !defined(MOZ_WIDGET_PHOTON)
  dosplash = PR_FALSE;
  for (i=1; i<argc; i++)
...
{
	// [ghzil] {
	// if there is an instance running already, detect it.
	HANDLE singleInstanceMutex = CreateMutex( NULL, FALSE, "ghzRunner.mutex" );
	if (GetLastError() == ERROR_ALREADY_EXISTS) {
		HWND hwndGhostzilla = FindWindow( "Ghostzilla", NULL );
		if (hwndGhostzilla != NULL) {
			ShowWindow( hwndGhostzilla, SW_SHOW );
		}
		return 0;
	}
	// } [ghzil]
